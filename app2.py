...
def app():
    ...
    if uploaded_file is not None:
        ...
        with st.spinner("Pre-processing the image..."):
            ...
            new_image_array, row_num, col_num = preprocess_input_image(
                input_image_array)
        ...
        with st.spinner("Making the prediction..."):
            ...
            output_pred = conv_float_int(
                combine_image(preds, row_num, col_num, original_width,
                              original_height, remove_ghost=True)[:, :, 0])
        ...
        output_mask = conv_float_int(
            combine_image(preds_t, row_num, col_num, original_width,
                          original_height, remove_ghost=False)[:, :, 0])
        ...
        forest_type = st.sidebar.selectbox(
            "Please select the type of forest: ", (
                ...
                'Shrublands',
                'Grasslands'))
        resolution = st.sidebar.text_input(
            "Please enter the image resolution value: ", '10')

        area, biomass_burnt, equal_days = burn_area(output_mask=output_mask,
                                                    resolution=float(
                                                        resolution),
        ...
        st.sidebar.text("{0:.2f}".format(
            equal_days) + " days of daily electricity power emission")
